...
//		mazeNo = 0;
	...
	public void movePackMan(int reqDir) {
		...
		if (move(reqDir, packman)) {
			...
			move(packman.curDir, packman);//if not keep it in its current direction
		}
    ...
    for (int g=0; g< 100; g++) {
	 int x2= packman.pos.row;
	  int x1= mazes[0].packmanPos.row;
		...
		gdistance[0] =  (int) Math.sqrt(plus);
}
	    int minValue = gdistance[0];
	    for (int i = 1; i < gdistance.length; i++) {
	        if (gdistance[i] < minValue) {
	            minValue = gdistance[i];
	        }
	    }
		return minValue;
	}
    
    public int HN(GameData data) {
	final int[] hdistance = null;
	...
	final int x1= powerPills.get(g).row;
	 final int x2= packman.pos.row;
		...
		final double plus= Math.pow(dx, 2)+ Math.pow(dy,2);
		 hdistance[g] = (int) Math.sqrt(plus) ;
}
    ...
    public List<Integer> getPossibleDirs(Position pos) {//returns the possible direction of the agent; up,right.down,up depend of the surrounding obstacles and store them in the list
		List<Integer> list = new ArrayList<Integer>();
		for (int d=0; d<4; d++) {
			Position npos = getNextPositionInDir(pos, d); //get the row and col for the new position
			if (mazes[mazeNo].charAt(npos.row, npos.column) != '0') {//if the raw of the new pos and col for the new position is not giving false direction(obstecle)
				list.add(d);//add the direction to the list 
			}
		}
		return list;
	}
    
	private Position getNextPositionInDir(Position pos, int d) {//it gives the row and col for the position from above function
		// determines the row and col for the above position 
