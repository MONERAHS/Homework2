...
public class PackMan extends Game {
	...
	GameData data;
	SpriteSheet drawer;
	GhostsCoach ghostsCoachاا;
	PackManCoach packmancoach;
	
	public PackMan() {//Initialise the game & its interface
		data = new GameData();
		drawer = new SpriteSheet("images/packman_sheet.png", "images/packman_sheet.info");
		ghostsCoach = new GhostsCoach();
		packmancoach = new PackManCoach(); 
		title = "PackMan";
		width = data.getWidth();
	...
	@Override
	public void update() {
		if (!data.dead) {//play as long as you are not dead
			frame++;
			data.movePackMan(packmancoach.decide(data));
			if (frame%2 == 0) {
	...
	public void draw(Graphics2D g) {
		...
		if (data.dead) {
			...
			g.fillRect(0, 0, width, height);
			drawer.draw(g, "over", 0, width/2, height/2-50, true);//draw 'OVER' on the screen 
		}
	}
}